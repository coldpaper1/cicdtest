pipeline {
  ...
  stages {
    ...
    stage('deploy k8s') {
      steps {
        sh '''
        sudo kubectl set image deployment deploy-blog ctn-blog=mhkim1560/testshop:newnewblog3
        sudo kubectl set image deployment deploy-shop ctn-shop=mhkim1560/testshop:newnewshop3
        '''
      }
    }
  }
